...
// the moment we move forward to the right, window gets bigger and bigger (ofc) and we
...
// if the the element to be insert is bigger then we shift the elements to the right and
...
int main()
{
    ...
    for (int i=1;i<n;i++){ 
        int j = i;
        while (j>0 && arr[j-1]>arr[j]){ // going till n-1 so that we can compare last element with second last
            int temp = arr[j];
            arr[j]= arr[j-1];
            arr[j-1] = temp;
            j--;
